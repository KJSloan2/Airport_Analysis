...
ucRef_ = pd.read_csv(str("%s%s" % (r"00_resources/","L_UNIQUE_CARRIERS.csv")),encoding="utf-8", low_memory=False)
...
aptRef_ = pd.read_csv("%s%s" % (r"00_resources/","REF_AIRPORT_TYPE.csv"),encoding="cp1252", low_memory=False)
...
actRef_ = pd.read_csv(str("%s%s" % (r"00_resources/","L_AIRCRAFT_TYPE.csv")),encoding="utf-8", low_memory=False)
...
airportRef_ = pd.read_csv(str("%s%s" % (r"00_resources/","AIRPORTS_GLOBAL.csv")),encoding="cp1252", low_memory=False)
...
for fname in files_:
	...
	data_ = pd.read_csv(str("%s%s" % (r"01_data/",fname)),encoding="cp1252")
...
with open(str("%s%s%s%s" % (r"02_output/","fleetMix_by_origin_","2012-2022",".json")), "w", encoding='utf-8') as json_output:
