...
for iataCode in get_iataCode:
	if iataCode in airportRef_locId:
		...
		for year in years:
			operationalStats_["location_metrics"][iataCode][year] = {
...
######################################################################################
with open(str("%s%s%s%s" % (r"airports/02_output/","operationalStats",str(years[0])+str(years[-1]),".json")), "w", encoding='utf-8') as json_output:
